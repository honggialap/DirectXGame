...
void CSuperLeaf::Load()
{
	...
	DROP_SPEED = move.attribute("DROP_SPEED").as_float();
	MOVE_SPEED = move.attribute("MOVE_SPEED").as_float();
	MOVE_LIMIT = move.attribute("MOVE_LIMIT").as_float();
...
void CSuperLeaf::Render()
{
	...
	case CSuperLeaf::EAction::MOVE:
		if (_left)	_sprites[SPR_SUPER_LEAF_LEFT]->Render(_x, _y);
		else		_sprites[SPR_SUPER_LEAF_RIGHT]->Render(_x, _y);
		break;
